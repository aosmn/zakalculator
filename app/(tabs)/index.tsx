...
function ToggleSwitch({
  ...
  const isRTL = lang === "ar";
  const onPos = isRTL ? 3 : 24;
  const offPos = isRTL ? 24 : 3;

  const thumbPos = useRef(new Animated.Value(value ? onPos : offPos)).current;
  const trackColor = useRef(new Animated.Value(value ? 1 : 0)).current;

  useEffect(() => {
    Animated.spring(thumbPos, {
      toValue: value ? onPos : offPos,
...
export default function AssetsScreen() {
  ...
  const [editingCurrency, setEditingCurrency] = useState<
    CurrencyHolding | undefined
  >();
  const [showMetalModal, setShowMetalModal] = useState(false);
  const [metalModalType, setMetalModalType] = useState<"gold" | "silver">(
    "gold",
  );
  ...
  const [balancesGroupMode, setBalancesGroupMode] =
    useState<BalancesGroupMode>("currency");
  ...
  function openAddGold() {
    setMetalModalType("gold");
    setEditingMetal(undefined);
    setShowMetalModal(true);
  }
  function openAddSilver() {
    setMetalModalType("silver");
    setEditingMetal(undefined);
    setShowMetalModal(true);
  }
  function openEditMetal(h: MetalHolding) {
    setEditingMetal(h);
    setMetalModalType(h.type);
    setShowMetalModal(true);
  }
  ...
  const sortedBalances = [...state.currencyHoldings].sort((a, b) =>
    a.label.localeCompare(b.label),
  );
  const sortedGold = state.metalHoldings
    .filter((h) => h.type === "gold")
    .sort((a, b) => a.label.localeCompare(b.label));
  const sortedSilver = state.metalHoldings
  ...
  const currenciesTotalBase = sortedBalances.reduce(
    (s, h) =>
      s + convertToBase(h.amount, h.currency, baseCurrency, exchangeRates),
    0,
  );
  const goldTotalValue = sortedGold.reduce(
    (s, h) =>
      s + goldValue(h, goldPricePerGram, goldPurityPrices?.[String(h.purity)]),
    0,
  );
  ...
  const silverTotalValue = sortedSilver.reduce(
    (s, h) => s + silverValue(h, silverPricePerGram),
    0,
  );
  ...
  function renderBalancesItems() {
    if (balancesGroupMode === "currency") {
      return groupBy(sortedBalances, (h) => h.currency).map(
        ({ groupKey: currency, items }) => {
          const totalAmount = items.reduce((s, h) => s + h.amount, 0);
          const totalBase = items.reduce(
            (s, h) =>
              s +
              convertToBase(h.amount, h.currency, baseCurrency, exchangeRates),
            0,
          );
          return (
            <View key={currency}>
    ...
    if (balancesGroupMode === "label") {
      return groupBy(sortedBalances, (h) => h.label).map(
    ...
    return (
      <>
        ...
        {sortedGold.length === 0 ? (
          ...
          <>
            {groupBy(sortedGold, (h) =>
              formatPurity(h.purity, h.purityUnit),
            ).map(({ groupKey, items }) => {
              ...
              const totalValue = items.reduce(
                (s, h) =>
                  ...
                  goldValue(
                    h,
                    goldPricePerGram,
                    goldPurityPrices?.[String(h.purity)],
                  ),
                0,
              );
              return (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.inner}>
          {showIntro && (
            ...
            >
              ...
              <View style={styles.introContent}>
                <Text style={[styles.introTitle, { color: text }]}>
                  {t("appIntroTitle")}
                </Text>
                <Text style={[styles.introDesc, { color: muted }]}>
                  {t("appIntroDesc")}
                </Text>
              </View>
              <Pressable
          ...
          >
            <ToggleSwitch
              ...
              onValueChange={(v) =>
                setShowTotals((prev) => ({ ...prev, [activeTab]: v }))
              }
            />
        ...
        <View style={[styles.tabBarWrap, { backgroundColor: bg }]}>
          <View style={styles.tabBarInner}>
            <View
              style={[styles.tabBar, { backgroundColor: card }, cardShadow]}
            >
              {TAB_CONFIG.map((tab) => {
      ...
      <ConfirmDeleteSheet
        ...
        onConfirm={() => {
          if (deleteCurrency) deleteCurrencyHolding(deleteCurrency.id);
          setDeleteCurrency(undefined);
        }}
      ...
      <ConfirmDeleteSheet
        ...
        onConfirm={() => {
          if (deleteMetal) deleteMetalHolding(deleteMetal.id);
          setDeleteMetal(undefined);
        }}
