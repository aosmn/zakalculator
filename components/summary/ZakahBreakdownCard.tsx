...
function StatCard({
  ...
  if (compact) {
    const barEl = !hero && topStrip
      ? <LinearGradient colors={gradient} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={styles.leftBar} />
      : !hero && !topStrip
        ? <View style={[styles.leftBar, { backgroundColor: gradient[0] + '40' }]} />
    ...
    const contentEl = (
      <View style={styles.compactContent}>
        <LinearGradient
          ...
          style={[styles.compactIcon, { shadowColor: iconShadowColor, shadowOpacity: 0.4, shadowRadius: 7, shadowOffset: { width: 0, height: 3 }, elevation: 5 }]}>
          <Feather name={iconName} size={16} color="#fff" />
        </LinearGradient>
        <View style={styles.compactText}>
          <Text style={[styles.compactLabel, { color: labelColor }]}>{label}</Text>
          {sub ? <Text style={[styles.compactSub, { color: subTextColor }]}>{sub}</Text> : null}
        </View>
    ...
    return (
      <View style={[styles.compactOuter, cardShadow]}>
        {hero ? (
          <LinearGradient colors={gradient} start={{ x: 0, y: 1 }} end={{ x: 1, y: 0 }} style={styles.compactInner}>
            {barEl}
            {contentEl}
          </LinearGradient>
        ) : (
  ...
  const content = (
    <View style={styles.statContent}>
      <LinearGradient
        ...
        style={[styles.iconWrap, { shadowColor: iconShadowColor, shadowOpacity: hero ? 0.35 : 0.4, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 6 }]}>
        <Feather name={iconName} size={18} color="#fff" />
      </LinearGradient>
      <Text style={[styles.statLabel, { color: labelColor }]}>{label}</Text>
      <Text style={[styles.statValue, { color: valueColor }]}>{value}</Text>
      {sub ? <Text style={[styles.statSub, { color: subTextColor }]}>{sub}</Text> : null}
    </View>
  );
  ...
  return (
    <View style={[styles.statOuter, cardShadow]}>
      {hero ? (
        <LinearGradient colors={gradient} start={{ x: 0, y: 1 }} end={{ x: 1, y: 0 }} style={styles.statInner}>
          {content}
        </LinearGradient>
      ) : (
        <View style={[styles.statInner, { backgroundColor: bgColor }]}>
          {topStrip && (
            <LinearGradient colors={gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.topStrip} />
          )}
          {content}
        </View>
      )}
