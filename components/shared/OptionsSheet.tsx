  ...
  async function handleExport() {
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        const fileUri = await FileSystem.StorageAccessFramework.createFileAsync(perms.directoryUri, fileName, 'application/json');
    ...
    if (Platform.OS === 'web') {
      ...
      input.onchange = async () => {
        ...
        try {
          ...
          if (!isValidState(parsed) && !isStoredAppData(parsed)) { showBackupStatus('error', t('invalidBackup')); return; }
          setPendingData(parsed);
        } catch { showBackupStatus('error', t('couldNotRead')); }
      };
      input.click();
      return;
    }
          ...
          <>
              ...
              <View style={styles.toggleRow}>
                <View style={styles.toggleLabel}>
                  ...
                  <Text style={[styles.toggleLabelText, { color: text }]}>{t('language')}</Text>
                </View>
                <View style={styles.pills}>
                  {(['en', 'ar'] as const).map((l) => (
                    <Pressable
                      key={l}
                      style={[styles.pill, { borderColor: lang === l ? tint : border, backgroundColor: lang === l ? tint + '18' : 'transparent' }]}
                      onPress={() => setLang(l)}>
                      <Text style={[styles.pillText, { color: lang === l ? tint : muted }]}>
                        {l === 'en' ? 'EN' : 'AR'}
                      </Text>
                    </Pressable>
                  ))}
              ...
              <View style={styles.toggleRow}>
                <View style={styles.toggleLabel}>
                  <Feather name={isDark ? 'moon' : 'sun'} size={15} color={muted} />
                  <Text style={[styles.toggleLabelText, { color: text }]}>Color Mode</Text>
                </View>
                <View style={styles.pills}>
                  {([false, true] as const).map((dark) => (
                    <Pressable
                      key={String(dark)}
                      style={[styles.pill, { borderColor: isDark === dark ? tint : border, backgroundColor: isDark === dark ? tint + '18' : 'transparent' }]}
                      onPress={() => { if (isDark !== dark) toggleTheme(); }}>
                      <Feather name={dark ? 'moon' : 'sun'} size={13} color={isDark === dark ? tint : muted} />
                    </Pressable>
                  ))}
                </View>
              </View>
              ...
              <View style={styles.toggleRow}>
                <View style={styles.toggleLabel}>
                  ...
                  <Text style={[styles.toggleLabelText, { color: text }]}>{t('backupRestore')}</Text>
                </View>
              </View>
              <Text style={[styles.backupDesc, { color: muted }]}>{t('backupDesc')}</Text>
              {pendingData ? (
                <>
                  <Text style={[styles.toggleLabelText, { color: text, marginBottom: 8 }]}>
                    {isStoredAppData(pendingData) ? t('replaceAllMsg') : t('replacePersonMsg')}
                  </Text>
                  <View style={styles.backupButtons}>
                    <Pressable style={[styles.backupBtn, styles.backupBtnOutline, { borderColor: border }]} onPress={() => setPendingData(null)}>
                      <Text style={[styles.backupBtnText, { color: muted }]}>{t('cancel')}</Text>
                    </Pressable>
                    <Pressable style={[styles.backupBtn, { backgroundColor: danger }]} onPress={confirmImport}>
                ...
                <View style={styles.backupButtons}>
                  <Pressable style={[styles.backupBtn, { backgroundColor: tint }]} onPress={handleExport}>
                    <Text style={styles.backupBtnText}>{t('exportBtn')}</Text>
                  </Pressable>
                  <Pressable style={[styles.backupBtn, styles.backupBtnOutline, { borderColor: border }]} onPress={handleImport}>
                    <Text style={[styles.backupBtnText, { color: text }]}>{t('importBtn')}</Text>
                  </Pressable>
                </View>
              )}
              {backupStatus && (
                <Text style={[styles.backupStatus, { color: backupStatus.type === 'success' ? tint : danger }]}>
...
const styles = StyleSheet.create({
  ...
  sheet: { borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 40, maxHeight: '70%' },
  ...
  backupBtn: { flex: 1, borderRadius: 10, paddingVertical: 12, alignItems: 'center' },
